...
#drop down + plot to each complaint departmrnt and reason
...
##groub by segment and each category
...
## repeated complaints by each customer
...
##DELIEVERY
mean_d = {
    'Mean': [4.789474, 4.561404, 4.368421, 4.754386],
...
# selected featuers to be showen as default
...
##Authentcation to open with username and passwoard
...
app.layout = html.Div(style={'font-family': 'Arial, sans-serif', 'padding': '20px','background-color': 'white', 'text-align': 'right', 'color': 'black', 'font-size': '22px','mergin-left':'20px','margin-right': '20px'}, children=[
    html.Div([
        ...
        html.H2(children='فتح الله فرع الرحاب' ,style={'font-weight': 'bold','text-align': 'right', 'color':'black','background-color': '#FF6000'})
    ],className="banner"),
    html.Hr(style={'border-top': '2px solid #999','border-radius': '10px'}),
    dcc.RadioItems(
        options=[
    ...
    html.Div([
        ...
        dcc.Graph(
            figure=px.bar(
                ...
                title='"One Visit" Segment by Month', **common_layout
            ),
    ...
    html.Div([
        dcc.Graph(
            figure=px.scatter(
                ...
                title='relation between recency & number of days', **common_layout
            ),
...
html.Div([
    ...
    dcc.Graph(id='cat-plot4', style={'display': 'inline-block', 'margin-left': '20px','border': '2px solid #ddd','border-radius': '10px'}),  # Adjust margin as needed
     ...
     dcc.Graph(
    figure = px.scatter(customer_purchase, x='duration', y='عدد الزيارات', title='Scatter Plot of Duration vs. عدد الزيارات',size='معدل الشراء',color_discrete_sequence=['#FF6000']
).update_layout(
    ...
    html.Div([
            html.Hr(style={'border-top': '3px solid #999','border-radius': '10px'}),
            html.H1(children='شكاوي فرع الرحاب',
...
def render_content(tab):
    if tab == 'tab-1':
        ...
        dcc.Graph(
            figure=px.bar(
                ...
                y=['Yes', 'No'],
                labels={'y': 'Count'},
                title='Comparison of Yes and No Data',
        ...
        ).update_layout(**common_layout),style={'border': '2px solid #ddd','margin-top':'20px','border-radius': '10px'}
    )
    ])
        ...
        dcc.Graph(figure=px.line(mean_f, x='Category', y='Mean', title='Mean Values for Categories',
                                  labels={'Mean': 'المتوسط'},color_discrete_sequence=['#FF6000']).update_layout(**common_layout),style={'margin-top':'20px','border': '2px solid #ddd','border-radius': '10px'}),
    ...
    ).update_layout(**common_layout),style={'width': '100%', 'height': '100%', 'border': '2px solid #ddd','margin-top':'20px','border-radius': '10px'}
)
    ...
    dbc.Row(
        dbc.Col(
           ...
           dcc.Graph(figure=px.line
             (mean_p, x='Category', y='Mean', title='Mean Values for Categories', labels={'Mean': 'المتوسط'},color_discrete_sequence=['#FF6000']).update_layout(**common_layout),style={'border': '2px solid #ddd','margin-top':'20px','border-radius': '10px'}),
    ...
    dbc.Row(
        dbc.Col(
           ...
           dcc.Graph(figure=px.line
             (mean_d, x='Category', y='Mean', title='Mean Values for Categories', labels={'Mean': 'المتوسط'},color_discrete_sequence=['#FF6000']).update_layout(**common_layout),style={'border': '2px solid #ddd','border-radius': '10px'}),
        ...
        ).update_layout(**common_layout),style={'margin-top':'20px','border': '2px solid #ddd','border-radius': '10px'
}
    )
...
def update_cat_plot(selected_cat):
    if selected_cat:
        ...
        figure1 = px.bar(
            ...
            title='Most Selling Categories for Loyal Segment').update_layout(common_layout
        )
        ...
        figure2 = px.bar(
            ...
            y='count', x='segments',color='القسم السلعي',
            color_discrete_sequence=custom_color_palette
            ,title='Most Selling Categories for Other Segments').update_layout(**common_layout
        )
